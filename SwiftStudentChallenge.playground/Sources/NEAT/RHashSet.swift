...
public class RHashSet<T>:Hashable, Codable where T : Hashable & Codable{
    ...
    public func remove(object: T){
        set.remove(object)
        data.remove(at: data.firstIndex(of: object)!) // as long as object is in data, there will only be one
    }
    
    public func addSorted(_ obj:T){
        for i in 0..<size(){
